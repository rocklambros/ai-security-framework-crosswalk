...
5. **Learned vs hand-tuned weights.** Side-by-side weight comparison, three-curve ROC (hand-tuned, logistic, ordinal) computed directly from stored coefficients, and two confusion matrices that explain why the production system kept the hand-tuned blend.
...
Fifty seven of the 119 expert pairs survive in v2, which is a 47.9 percent preservation rate. Sixty two are lost, and fifty two are new. None of the preserved pairs flipped tier. By the session 11 rule of thumb that regression on more than five percent of v1 pairs should be flagged for manual review, this is a regression, and the lost set is the natural queue for the next active-learning round. The full written analysis lives at [`data/processed/V1_VS_V2_COMPARISON_REPORT.md`](data/processed/V1_VS_V2_COMPARISON_REPORT.md).
