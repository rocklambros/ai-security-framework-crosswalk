...
The on-disk ``pool_v1.jsonl`` is the ground truth. The report is computed
against whatever depth that artifact was persisted at; downstream stages
read the same file, so the floor numbers reflect what they will see.
"""
from __future__ import annotations
import json
from collections import defaultdict
import pandas as pd
from classifier.config import CANDIDATES_DIR, SPLITS_DIR
from classifier.data.candidates import FRAMEWORK_PAIRS, build_candidate_pool


def _load_pool_from_disk() -> dict[str, list[dict]]:
    path = CANDIDATES_DIR / "pool_v1.jsonl"
    pool_by_pair: dict[str, list[dict]] = defaultdict(list)
    with path.open() as f:
        for line in f:
            rec = json.loads(line)
            pair_key = rec.pop("framework_pair")
            pool_by_pair[pair_key].append(rec)
    return dict(pool_by_pair)
...
def check_floor(
    ...
    if from_disk:
        pool_by_pair = _load_pool_from_disk()
    else:
