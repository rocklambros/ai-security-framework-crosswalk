...
def test_every_framework_geq_2_source_and_target():
    sources = [p[0] for p in FRAMEWORK_PAIRS]
    targets = [p[1] for p in FRAMEWORK_PAIRS]
    for fw in FRAMEWORKS:
        assert sources.count(fw) >= 2, f"{fw} is source <2 times"
        assert targets.count(fw) >= 2, f"{fw} is target <2 times"
```
