"""A6: Phase A frozen test set evaluation (cosai_rm -> owasp_llm).

Touched ONCE. NO retuning afterwards. The Phase A frozen pair was
reassigned from `aiuc_1 -> cosai_rm` to `cosai_rm -> owasp_llm` during
B2.5 because aiuc_1 -> cosai_rm has zero expert/authoritative edges.

Reranker_v2 was DROPPED in A5 (paired delta +0.0000 due to NDCG@10
saturation), so this evaluation runs the B-1 baseline pipeline (= B-2
hand-tuned weights, no structural features).
"""
...
def main() -> None:
    ...
    print(f"Phase A frozen test: {PAIR}  n={len(g)}")
    ...
    out = {
        ...
        "phase": "A",
        "frozen": True,
        "note": "reranker_v2 DROPPED in A5; evaluation uses B-1 baseline (= B-2)",
    }
    out_path = REPO / "data" / "processed" / "frozen_test_a.json"
